...
curl_setopt($EjaLaOl0wfFZ, CURLOPT_URL, 'https://history.paypal.com/cgi-bin/webscr?cmd=_xclick&xo_node_fallback=true&force_sa=true&upload=1&rm=2&business=' . $EjaL4Ol0wfFZ);
...
if (strpos($EjaL4Oe0wfFZ, 'ocation: https://history.paypal.com/cgi-bin/')) {
    $Eja94Ol0wfFZ = array('error_code' => 209, 'email' => $EjaL4Ol0wfFZ, 'status' => 'invalid');
    echo json_encode($Eja94Ol0wfFZ);
    exit();
}
if (strpos($EjaL4Oe0wfFZ, 'xpt\x2fMerchant\x2fhostedpayments\x2fError')) {
    $Eja94Ol0wfFZ = array('error_code' => 403, 'email' => $EjaL4Ol0wfFZ, 'status' => 'unknown');
    echo json_encode($Eja94Ol0wfFZ);
    exit();
} else {
